...
func (h *HighPerformanceHooks) calcIRQSMPAffinityCPUs(ctx context.Context, containerServer *lib.ContainerServer,
	...
	log.Debugf(ctx, "DEBUG_FLAG container %q isdelete %t eligibleCPUSet %q", c.ID(), isDelete, eligibleCPUSet)
	...
	log.Debugf(ctx, "DEBUG_FLAG all cri-o containers %v", containers)
	...
	for _, container := range containers {
		...
		if container.ID() == c.ID() {
			log.Debugf(ctx, "DEBUG_FLAG skipped container %q because it shall be deleted", container.Name())

			continue
		}
		// Skip if container is not a static IRQ SMP affinity container.
		// Also checks for isContainerCPUsSpecEmpty so we can safely access the CPUs below.
		if !isIRQSMPStaticContainer(ctx, containerServer, container) {
			continue
		}

		containerSet, err := cpuset.Parse(container.Spec().Linux.Resources.CPU.Cpus)
		if err != nil {
		...
		log.Debugf(ctx, "DEBUG_FLAG before container %s enabledCPUSet: %s, containerSet: %s", container.ID(), enabledCPUSet, containerSet)
		enabledCPUSet = enabledCPUSet.Difference(containerSet)
		log.Debugf(ctx, "DEBUG_FLAG after container %s with CPUs %s enabled CPU set is %s", container.ID(), container.Spec().Linux.Resources.CPU.Cpus, enabledCPUSet)
...
func (h *HighPerformanceHooks) updateNewIRQSMPAffinityMask(ctx context.Context, containerServer *lib.ContainerServer,
	...
	log.Debugf(ctx, "DEBUG_FLAG IRQ affinity CPU sets - enabled: %q, disabled: %q", enabledCPUSet, disabledCPUSet)
...
func isIRQSMPStaticContainer(ctx context.Context, containerServer *lib.ContainerServer, c *oci.Container) bool {
	...
	if c.State().Status == specs.StateStopped {
		log.Debugf(ctx, "DEBUG_FLAG skipped container %q because it's not running", c.Name())
	}

	if !shouldIRQLoadBalancingBeDisabled(ctx, sb.Annotations()) {
		log.Debugf(ctx, "DEBUG_FLAG skipped container %q because IRQ loadbalancing not disabled %q", c.Name(), sb.Annotations())
	}

	if !isHighPerformanceRuntime(sb.RuntimeHandler(), sb.Annotations()) {
		log.Debugf(ctx, "DEBUG_FLAG skipped container %q because no high perf runtime %q, %q", c.Name(), sb.RuntimeHandler(), sb.Annotations())
	}

	if isContainerCPUsSpecEmpty(&cSpec) {
		log.Debugf(ctx, "DEBUG_FLAG skipped container %q because CPU.Cpus is empty", c.Name())
	}

	if !shouldRunHooks(ctx, c.ID(), &cSpec, sb) {
		log.Debugf(ctx, "DEBUG_FLAG skipped container %q because hooks disabled", c.Name())
	}
