	...
	Describe("UpdateIRQSmpAffinityMask", func() {
		...
		DescribeTable("testing cpu mask",
			...
			Entry("clear a single bit that was one", TestData{
				input:    Input{unmanagedCPUSet: makeCPUSet("1-47"), disabledCPUSet: cpuset.New(0), originalIRQSMPSetting: "0000,00003003"},
				expected: Expected{mask: "00000000,00003002", invMask: "0000ffff,ffffcffd"},
			}),
			Entry("set a single bit that was zero", TestData{
				input:    Input{unmanagedCPUSet: makeCPUSet("0-3,5-47"), enabledCPUSet: cpuset.New(4), originalIRQSMPSetting: "0000,00003003"},
				expected: Expected{mask: "00000000,00003013", invMask: "0000ffff,ffffcfec"},
			}),
			Entry("clear a set of bits", TestData{
				input:    Input{unmanagedCPUSet: makeCPUSet("0-3,14-47"), disabledCPUSet: makeCPUSet("4-13"), originalIRQSMPSetting: "ffff,ffffffff"},
				expected: Expected{mask: "0000ffff,ffffc00f", invMask: "00000000,00003ff0"},
			}),
			Entry("set a set of bits", TestData{
				input:    Input{unmanagedCPUSet: makeCPUSet("0-3,14-47"), enabledCPUSet: makeCPUSet("4-13"), originalIRQSMPSetting: "ffff,ffffc00f"},
			...
			Entry("clear a single bit that was one when odd mask is present", TestData{
				input:    Input{unmanagedCPUSet: makeCPUSet("0-8,10-11"), disabledCPUSet: cpuset.New(9), originalIRQSMPSetting: "fff"},
				expected: Expected{mask: "00000dff", invMask: "0000f200"},
			}),
			Entry("clear two bits from a short mask", TestData{
				input:    Input{unmanagedCPUSet: makeCPUSet("0-1,4-23"), disabledCPUSet: cpuset.New(2, 3), originalIRQSMPSetting: "ffffff"},
